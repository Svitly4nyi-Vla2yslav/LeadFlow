...
export default function LanguageSwitcher(){ const { i18n } = useTranslation(); const cur = i18n.language as 'de'|'uk'|'ru';
  return (<Wrap aria-label="Language">{(['de','uk','ru'] as const).map(l=><Lang key={l} active={cur===l} aria-pressed={cur===l} onClick={()=>i18n.changeLanguage(l)}>{l.toUpperCase()}</Lang>)}</Wrap>);
}
