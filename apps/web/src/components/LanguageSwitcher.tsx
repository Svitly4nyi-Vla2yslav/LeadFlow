...
const Wrap = styled.div`display:flex;gap:8px;align-items:center;background:${({theme})=>theme.colors.card};border:1px solid ${({theme})=>theme.colors.border};padding:6px 10px;border-radius:999px;@media(max-width:400px){gap:4px;padding:4px 6px}`;
const Lang = styled.button<{$active?:boolean}>`min-width:44px;min-height:44px;background:${({$active,theme})=>$active?theme.colors.accentSoft:'transparent'};color:${({theme})=>theme.colors.text};border:1px solid ${({theme})=>theme.colors.border};border-radius:999px;padding:6px 8px;cursor:pointer;`;
