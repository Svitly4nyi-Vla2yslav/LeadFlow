...
export default function DataManagement() {
  ...
  return <div className="page-stack">
    ...
    <Card><h2>{t('import.title')}</h2><p className="muted">{t('import.instructions')}</p><label>{t('import.file')}<input type="file" accept=".json,.csv,application/json,text/csv" onChange={event => chooseFile(event.target.files?.[0])} /></label>
      ...
      {result && <div aria-live="polite"><p>{t('import.result', { created: result.created, skipped: result.skipped, errors: result.errors.length })}</p>{!!result.errors.length && <><h3>{t('import.errors')}</h3><ul>{result.errors.map(item => <li key={`${item.row}-${item.error}`}>{t('import.rowError', { row: item.row, error: t('import.failed') })}</li>)}</ul></>}</div>}
