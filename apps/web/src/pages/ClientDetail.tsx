...
const feedbackValue = (key: keyof NonNullable<CallTask['result']>, value: unknown, language: string, t: (key: string) => string) => {
  if (Array.isArray(value)) return value.join(' · ');
  if (key === 'interestLevel' && typeof value === 'string') return t(`interestLevel.${value}`);
  if (typeof value === 'string' && (/At$|Start$|End$/.test(key))) {
...
export default function ClientDetail() {
  ...
  return <div className="page-stack client-detail">
    ...
    <Section title={t('sections.lastFeedback')} open={!!result}>{!result ? <p className="muted">{t('feedback.empty')}</p> : <dl className="feedback-grid">{feedbackFields.map(([key,label])=>{const value=result[key];if(value===undefined||value===''||(Array.isArray(value)&&!value.length))return null;return <div key={key}><dt>{t(label)}</dt><dd>{feedbackValue(key,value,i18n.language,t)}</dd></div>})}{result.objections?.length?<div><dt>{t('feedback.objections')}</dt><dd>{result.objections.join(' · ')}</dd></div>:null}{result.doNotContact!==undefined?<div><dt>{t('feedback.doNotContact')}</dt><dd>{result.doNotContact?t('common.yes'):t('common.no')}</dd></div>:null}</dl>}</Section>
