...
export default function Leads() {
  ...
  const callWithEmma = async (task: CallTask) => {
    if (launchingLeadId) return;
    setLaunchingLeadId(task.leadId); setError('');
    try {
      const response = await api.post('/api/voice-agent/handoff', { leadId: task.leadId, callTaskId: task.id });
