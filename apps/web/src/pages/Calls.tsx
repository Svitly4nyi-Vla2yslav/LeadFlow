...
export default function Calls(){
  ...
  const callWithEmma=async(task:CallTask)=>{if(launchingLeadId)return;setLaunchingLeadId(task.leadId);setError('');try{const response=await api.post('/api/voice-agent/handoff',{leadId:task.leadId,callTaskId:task.id});const destination=new URL(response.data.voiceAgentAppUrl);destination.searchParams.set('handoff',response.data.handoffToken);const opened=window.open(destination.toString(),'_blank');if(!opened)throw new Error('popup_blocked');opened.opener=null}catch(exception:any){setError(voiceAgentLaunchError(exception,t))}finally{setLaunchingLeadId('')}};
  return <div className="page-stack"><Card><h1>{t('calls.title')}</h1><p className="muted">{t('calls.subtitle')}</p>{error&&<p role="alert" className="error-text">{error}</p>}</Card><div className="call-task-list">{tasks.map(task=>{const lead=leads[task.leadId];const canCall=task.status==='READY'&&!task.readinessIssues.length;return <Card key={task.id}><div className="lead-card-head"><div><h3>{lead?.company||t('lead.unknown')}</h3><p>{task.scheduledAt?new Date(task.scheduledAt).toLocaleString(i18n.language):t('call.notScheduled')}</p></div><span className="status-pill">{t(`callStatus.${task.status}`)}</span></div><p>{task.callObjective||t('readiness.missing_call_objective')}</p>{canCall?<button className="action-link primary card-action-button" type="button" disabled={!!launchingLeadId} onClick={()=>callWithEmma(task)}>{launchingLeadId===task.leadId?t('voiceAgent.openingEmma'):t('call.callEmma')}</button>:<Link className="action-link primary" to={`/clients/${task.leadId}#emma`}>{t('call.prepare')}</Link>}</Card>})}{!tasks.length&&!error&&<Card>{t('calls.empty')}</Card>}</div></div>;
}
