...
Netlify serves the Vite application and routes `/api/*` to the bundled Express function. The function keeps the CRM JSON document in the strongly consistent `leadflow-crm` Netlify Blobs store, so production no longer depends on a visitor's `localhost:3001`.
