...
Додаток уже придатний як локальний операційний CRM для однієї людини. Він ще не готовий як публічна багатокористувацька production-система: для цього потрібні авторизація, серверна база даних, резервні копії, захист API, інтеграції зв’язку та узгоджена політика персональних даних.
...
1. **Авторизація й права доступу.** Екрани Login/Settings поки не захищають API. Потрібні реальна сесія, безпечні cookies, logout, відновлення доступу й хоча б ролі Owner/Viewer.
